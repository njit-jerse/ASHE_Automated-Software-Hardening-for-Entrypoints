 ...
 *
 * <p>This class provides methods for extracting method signature, method bodies, class
 * declarations, and Java code blocks from a given string or file.
 */
public final class JavaCodeParser {
  ...
  /**
   * Represents the signature of a Java method. This includes information about its access modifier,
  ...
  public record MethodSignature(
       ...
       * Indicates the presence or absence of a modifier in a method signature.
       *
       * <p>Used to determine whether to add specific modifiers to a {@link MethodDeclaration}. If
       * the value is {@code ABSENT}, no modifiers are added to the method, reflecting the default
       * or package-private access. If the value is {@code PRESENT}, the associated modifiers from
       * {@code modifierKeyword()} are added to the method.
       */
      ModifierPresent modifierPresent,
      /**
       * A list of access modifiers for the method. It can be empty, indicating that the method has
      ...
      String methodName,
      /**
       * The method's parameters, formatted as a comma-separated string. Each parameter comprises
       * its type followed by its name, e.g., "int x, String y". This string is typically processed
       * further to delineate individual parameter nodes when generating a {@link
       * MethodDeclaration}.
       */
      String parameters) {
     ...
     *
     * @return {@code true} if the method signature contains non-null return type, method name, and
     *     parameters, representing a valid Java method signature; {@code false} otherwise.
     */
    public boolean isValid() {
      boolean isValidSig =
          this.returnType() != null && this.methodName() != null && this.parameters() != null;

      if (isValidSig) {
        LOGGER.info(
            "Java method signature is valid: returnType={}, methodName={}, parameters={}",
            this.returnType(),
            this.methodName(),
            this.parameters());
      } else {
        LOGGER.warn(
            "Invalid Java method signature detected: returnType={}, methodName={}, parameters={}",
            this.returnType(),
            this.methodName(),
            this.parameters());
      }
  ...
  public static MethodSignature extractMethodSignature(String method)
    ...
    try {
      ...
      LOGGER.info(
          "Extracted method signature: Modifiers={} ReturnType={} Name={} Parameters={}",
          modifiers.isEmpty()
              ? "None"
              : modifiers.stream().map(Enum::name).collect(Collectors.joining(", ")),
          returnType,
          name,
          params);
   ...
   *     indicating no other modifiers are to be considered. Returns {@code ModifierPresent.PRESENT}
   *     otherwise.
   */
  private static ModifierPresent determineModifierPresence(List<Modifier.Keyword> keywords) {
    return keywords.contains(Modifier.Keyword.DEFAULT)
        ? ModifierPresent.ABSENT
        : ModifierPresent.PRESENT;
  }

  /**
   * Extracts and returns the parameters from the provided method declaration as a formatted string.
   *
  ...
  /**
   * Extracts the modifier keyword(s) from a given {@link MethodDeclaration}. Returns {@link
   * Modifier.Keyword#DEFAULT} if no specific modifier is found.
   *
  ...
  private static List<Modifier.Keyword> extractModifiers(MethodDeclaration methodDeclaration) {
    List<Modifier.Keyword> keywords =
   ...
   * </ul>
   *
   * <p>If the target method string does not match the expected format, this method throws an {@link
   * IllegalArgumentException}.
   *
   * @param targetMethod the target method string in the format
  ...
  public static String extractMethodName(String targetMethod) throws IllegalArgumentException {
    ...
    if (matcher.find()) {
      ...
      // Explanation: In the context of the regex pattern "#(.*?)\\(", `group(0)` would represent
      // the entire
      // matched substring starting from `#` and ending just before the first open parenthesis `(`.
      // So, for an input like "com.example.package.MyClass#myMethod(ParamType1, ParamType2)",
      // `group(0)`
      // would return `#myMethod(`, while `group(1)` captures just the method name, returning
      // `myMethod`.
  ...
  /**
   * Finds the class or interface declaration containing a method with a specified name from a Java
   * file.
   *
