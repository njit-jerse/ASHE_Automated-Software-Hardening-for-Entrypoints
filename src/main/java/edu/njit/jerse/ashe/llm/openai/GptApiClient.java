...
/**
 * Provides functionality to interact with the GPT API, facilitating the fetching of corrections
 * based on provided prompts. It encapsulates the process of constructing API requests, sending
 * them, and handling the responses.
 */
public class GptApiClient extends AbstractApiClient {
  ...
  private final String GPT_SYSTEM_CONTENT =
      config.getPropertyValue("openai.message.system.content");
   ...
   * @return a {@code GPTRequest} object configured with the necessary parameters for the GPT API
   *     call
   */
  ...
  /**
   * Creates an HTTP request object suitable for the GPT API, configured with the required headers
   * and body.
   *
   * @param apiRequestBody a {@code String} containing the JSON representation of the GPT request
   *     object
  ...
  /**
   * Handles the response received from the GPT API, extracting the model's output from the response
   * body.
   *
   * @param httpResponse the {@code HttpResponse<String>} object containing the API's response
   * @return a {@code String} representing the model's output, or an error message in case of
  ...
  public String handleApiResponse(HttpResponse<String> httpResponse) throws IOException {
    ...
    if (httpResponse.statusCode() == 200) {
      ...
      String errorMsg =
          "Error:" + System.lineSeparator() + httpResponse.statusCode() + " " + httpResponse.body();
