...
public class OpenAiRequestHandler implements ApiRequestHandler {
   ...
   *     response
   * @throws TimeoutException if the CompletableFuture does not complete within {@link
   *     gptResponseTimeout}
   */
  @Override
  public HttpResponse<String> apiResponse(HttpRequest request, HttpClient client)
    ...
    try {
      ...
      LOGGER.fatal(
          "API response took too long to be received (over " + gptResponseTimeout + " seconds)");
      throw e;
    } finally {
      executor.shutdown();
    }
